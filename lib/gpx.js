...
const attributeNames = ["speed", "course", "hAcc", "vAcc"];
...
function coordPair(x) {
  ...
  if (extensions !== null) {
    for (let i = 0; i < attributeNames.length; i++) {
      ...
      const v = parseFloat(get1(extensions, name));
      if (!isNaN(v)) {
...
function getPoints(node, pointname) {
  ...
  for (let i = 0; i < l; i++) {
    ...
    for (let j = 0; j < c.extendedValues.length; j++) {
      const name = c.extendedValues[j][0];
      const val = c.extendedValues[j][1];
      if (!extendedValues[name]) {
        extendedValues[name] = Array(pts.length).fill(null);
      }
      extendedValues[name][i] = val;
    }
  }
  const result = {
    line: line,
      ...
      attributeNames.forEach((name) => {
        if (
          ...
          if (line[name] && line[name].length) {
            ...
            extendedValues[name].push(Array(line.line.length || 0).fill(null));
          }
        }
      });
    }
